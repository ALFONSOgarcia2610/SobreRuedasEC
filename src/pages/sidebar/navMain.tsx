  ...
  return (
    <SidebarGroup>
      <SidebarGroupLabel className=" flex items-center gap-2 text-slate-200">
        ...
        <div
          className={`w-2 h-2 rounded-full bg-current ${roleConfig.color} opacity-60`}
        ></div>
      </SidebarGroupLabel>
      <SidebarMenu>
        {items.map((item) => {
              ...
              <SidebarMenuItem>
                ...
                {item.url ? (
                  ...
                  <SidebarMenuButton
                    asChild
                    tooltip={item.title}
                    className="hover:bg-slate-800/50 text-slate-200 hover:text-white"
                  >
                    ...
                    <SidebarMenuSub>
                      {item.items?.map((subItem) => (
                        <SidebarMenuSubItem key={subItem.title}>
                          <SidebarMenuSubButton
                            asChild
                            className="hover:bg-slate-800/30 text-slate-300 hover:text-white"
                          >
                            <Link to={subItem.url}>

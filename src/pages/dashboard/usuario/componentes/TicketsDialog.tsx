              ...
              {(queryTickets.data ?? []).map((ticket, idx) => {
                ...
                return (
                  <div
                    ...
                    style={{ boxShadow: '0 4px 24px 0 rgba(53,58,77,0.18), 0 1.5px 0 0 #353a4d inset' }}
                  >
                    {/* Logo y estrellas pastel */}
                    <div className="flex flex-col items-center justify-center w-full pt-1 pb-0 z-10">
                      <img src="/public/img/logoSR.png" alt="Logo" className="w-6 h-6 mb-2 drop-shadow-[0_1px_2px_rgba(80,80,120,0.7)]" />
                        <div className="flex items-center gap-2 mb-0.5">
                          <Star className="w-3 h-3 text-[#7dd3fc] opacity-80" />
                          <Sparkles className="w-3 h-3 text-[#fbcfe8] opacity-80" />
                          <Star className="w-3 h-3 text-[#a5b4fc] opacity-80" />
                        </div>
                    </div>
                    {/* Número o texto */}
                    <span className="text-sm font-semibold mt-2 px-2 py-1 rounded-lg bg-[#23263a] text-[#fbcfe8] border border-[#353a4d] shadow-sm">
                      {noAsignado ? 'Pendiente' : <span className="inline-flex items-center gap-1"><Hash strokeWidth={1.5} className="w-4 h-4 text-[#7dd3fc]" /> {t.number}</span>}
                    </span>
                  </div>
                );
              })}

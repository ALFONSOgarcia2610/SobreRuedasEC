...
import { Calendar, DollarSign, Hash, Ticket, TrendingUp, Package, Tag } from 'lucide-react';

export default function SorteoActivo() {
  const { data: currentLottery, isLoading, isError, error } = useGetCurrentLottery();
  const { data: products, isLoading: isLoadingProducts } = useGetProductsByLotteryId(currentLottery?.lotteryId);
  ...
  if (!currentLottery) {
    return (
      <Card className="bg-slate-900 border-slate-800">
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-yellow-400">No hay sorteo activo</CardTitle>
          <CardDescription className="text-slate-400">
  ...
  const ticketsVendidos = 0; // TODO: Obtener de la API cuando esté disponible
  const porcentajeVendido = currentLottery.maxTickets > 0 
      ...
      <CardContent className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <Badge variant="outline" className="mt-1 bg-green-600/20 text-green-400 border-green-600">
                Activo
              </Badge>
            </div>
          </div>
          ...
          <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-wide">Número</p>
                <p className="text-2xl font-bold text-white">{currentLottery.number}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-wide">Precio Voucher</p>
          ...
          <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-wide">Máx. Tickets</p>
                <p className="text-2xl font-bold text-white">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            <span className="text-slate-300 font-medium">Progreso de ventas</span>
            <span className="text-green-400 font-bold">{porcentajeVendido}%</span>
          </div>
      ...
      <CardContent>
        {isLoadingProducts ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-slate-400 text-lg">No hay productos asociados a este sorteo</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {products.map((product) => (
              <div
                key={product.id}
                className="bg-slate-800/50 rounded-lg p-5 border border-slate-700 hover:border-blue-500/50 hover:shadow-lg hover:shadow-blue-500/10 transition-all duration-300 group"
              >
                {/* Header del producto */}
                <div className="flex items-start justify-between mb-3">
                  ...
                  >
                    {product.isCash ? 'Efectivo' : 'Premio'}
                  </Badge>
                </div>
                ...
                <div className="flex items-center gap-2 pt-3 border-t border-slate-700">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold text-green-400">
                      ${product.value.toLocaleString('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </p>
                  </div>
                </div>
              </div>

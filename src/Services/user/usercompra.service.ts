...
export const createVoucherService = async (voucherData: CreateVoucherDto): Promise<Voucher> => {
  const response = await networkClient.post<{
    success: boolean;
    message: string;
    data: Voucher;
  }>(
    `${envs.VITE_API_URL}/api/Voucher`,
    voucherData
  );
...
export const createTicketService = async (ticketData: CreateTicketDto): Promise<Ticket> => {
...
export const getAllVoucherbyUserService = async (): Promise<Voucher[]> => {
  const response = await networkClient.get<{ 
    ...
    `${envs.VITE_API_URL}/api/Voucher/my-vouchers`
  );
...
export const getAllTicketsByVoucherService = async (voucherId: string): Promise<Ticket[]> => {
  const response = await networkClient.get<{ 
    success: boolean; 
    message: string; 
    data: Ticket[];
...
export const getAllEntityFinanceService = async (id: string): Promise<string[]> => {
  const response = await networkClient.get<{ 
    success: boolean; 
    message: string; 
    data: string[];
  ...
  if (!response || !response.success || !response.data) {
    throw new Error(response?.message || 'Error al obtener entidades financieras');
  }
  return response.data;
};
